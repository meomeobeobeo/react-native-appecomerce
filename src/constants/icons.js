const Home = require('../../assets/icons/Home.png')
const Search = require('../../assets/icons/Search.png')
const Favorite = require('../../assets/icons/Favorite.png')
const FavoriteFilled = require('../../assets/icons/Favorite_filled.png')
const Hamburger = require('../../assets/icons/Hamburger.png')
const Notification = require('../../assets/icons/Notification.png')
const ArrowLeft = require('../../assets/icons/Arrow_Left.png')
const Location = require('../../assets/icons/Location.png')
const Filter = require('../../assets/icons/Filter.png')
const Back = require('../../assets/icons/back.png')
const User = require('../../assets/icons/User.png')
const Logo = require('../../assets/images/catmoon_rm.png')
const FaceBookLogo = require('../../assets/icons/facebook.png')
const GoogleLogo = require('../../assets/icons/google.png')
const AppleLogo = require('../../assets/icons/apple-logo.png')
const VisibleLogo = require('../../assets/icons/Visible.png')
const HideLogo = require('../../assets/icons/hide.png')
const BellIcon = require('../../assets/icons/bellIcon.png')
const CartIcon = require('../../assets/icons/cartIcon.png')
const LogoNotText = require('../../assets/icons/LogoNotText.png')
const Book = require('../../assets/icons/Book.png')
const Car = require('../../assets/icons/Car.png')
const Exterior = require('../../assets/icons/Exterior.png')
const SoccerBall = require('../../assets/icons/SoccerBall.png')
const Technology = require('../../assets/icons/Technology.png')
const Star = require('../../assets/icons/Start.png')
const StarHalf = require('../../assets/icons/StartHalf.png')
const Scales = require('../../assets/icons/Scales.png')
const Cartblack = require('../../assets/icons/CartBlack.png')
const ArrowRight = require('../../assets/icons/ArrowRight.png')

const Credit = require('../../assets/icons/credit-card.png')
const Information = require('../../assets/icons/information.png')
const Language = require('../../assets/icons/language.png')
const Logout = require('../../assets/icons/logout.png')
const Map = require('../../assets/icons/maps-and-flags.png')
const Quest = require('../../assets/icons/question.png')
const ShoppingBag = require('../../assets/icons/shopping-bag.png')

const Camera = require('../../assets/icons/Camera.png')
const Calendar = require('../../assets/icons/calendar.png')
const EditText = require('../../assets/icons/edit-text.png')
const Address = require('../../assets/icons/Address.png')
const CreditCard = require('../../assets/icons/credit-cards.png')
const DigitalWallet = require('../../assets/icons/digital-wallet.png')
const Visa = require('../../assets/icons/visa-logo.png')
const MasterCard = require('../../assets/icons/master-card.png')
const Fashion = require('../../assets/icons/Fashion.png')
const Beauty = require('../../assets/icons/skin-care.png')
const Category = require('../../assets/icons/Category.png')
const LiveStream = require('../../assets/icons/LiveStream.png')
const FavoriteFill = require('../../assets/icons/heart.png')
const UserFill = require('../../assets/icons/UserInfor.png')
const SearchWhite = require('../../assets/icons/SearchWhite.png')
const HouseWare = require('../../assets/icons/houseware.png')
const SkinCare = require('../../assets/icons/skin-care.png')
const Food = require('../../assets/icons/Food.png')
const Shoes = require('../../assets/icons/Shoes.png')
const ToyStore = require('../../assets/icons/ToyStore.png')
const HealthCare = require('../../assets/icons/healthcare.png')
const Share = require('../../assets/icons/share.png')
const Next = require('../../assets/icons/next.png')
const Transport = require('../../assets/icons/van-cargo.png')
const MoreInfor = require('../../assets/icons/Ellipsis.png')
const ShareFill = require('../../assets/icons/ShareFill.png')
const GoBack = require('../../assets/icons/goBackFill.png')
const AddShoppingCard = require('../../assets/icons/AddShoppingCart.png')
const ShoppingCard = require('../../assets/icons/shopingCard.png')
const Plus = require('../../assets/icons/plus.png')
const Minus = require('../../assets/icons/minus-sign.png')



const Chat = require('../../assets/icons/Chat.png')




export default {
    Home,
    FavoriteFilled,
    Favorite,
    Hamburger,
    Search,
    Notification,
    ArrowLeft,
    Location,
    Filter,
    Back,
    User,
    Logo,
    FaceBookLogo,
    GoogleLogo,
    AppleLogo,
    VisibleLogo,
    HideLogo,
    BellIcon,
    CartIcon,
    LogoNotText,
    Book,
    Car,
    Exterior,
    SoccerBall,
    Technology,
    Star,
    StarHalf,
    Scales,
    Cartblack,
    ArrowRight,
    Credit,
    Information,
    Language,
    Logout,
    Map,
    Quest,
    ShoppingBag,
    Camera,
    Calendar,
    EditText,
    Address,
    CreditCard,
    DigitalWallet,
    Visa,
    MasterCard,
    Fashion,
    Beauty,
    Category,
    LiveStream,
    FavoriteFill,
    UserFill,
    SearchWhite,
    HouseWare,
    SkinCare,
    Food,
    Shoes,
    ToyStore,
    HealthCare,
    Share,
    Next,
    Transport,
    MoreInfor,
    ShareFill,
    GoBack,
    AddShoppingCard,
    Chat,
    ShoppingCard,
    Plus,
    Minus
}
